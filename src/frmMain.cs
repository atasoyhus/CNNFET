...
namespace CCNFET
{
    public partial class frmMain : Form
    {
        ...
        private void scanFiles()
        {
            ...
            new Thread(() =>
            {
                dataset = new Dataset(selectedPath, maxNumOfInsPerClass);

                if (maxNumOfInsPerClass > 0)
                    appendLine("Max number of instance per class: " + maxNumOfInsPerClass, "");
        ...
        private void btnExtract_Click(object sender, EventArgs e)
        {
            ...
            if (
                layerIndex < lastConvLayerInd &&
                MessageBox.Show(this, "It is recommended to select one of the last convolution layers (right before the fully connected layers which seems to be the layer #" + lastConvLayerInd + ". Note that fully connected layers may be implemented as convolution layers with 1x1 filters). Although shallow layers has more output, the features captured by those layers may be less distiguishing. Do you want to continue?", "Convolution Layer", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes
            )
                return;
        ...
        private void tabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (selectedTab == tpExport || selectedTab == tpClassification)
                {
